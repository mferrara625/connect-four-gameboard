...
//        gameboard.a6 = "x";
    ...
    public static void play(Gameboard gameboard, int i) {
        System.out.println(gameboard);
        Scanner scan = new Scanner(System.in);
        System.out.println("Pick a column to drop your piece into (1 - 7) : ");
        int input = scan.nextInt();
        for (int j = gameboard.position.length - 1; j >= 0; j--) {
            for (int k = 0; k < gameboard.position[j].length; k++) {
                if (input - 1 == k && gameboard.position[j][k].equalsIgnoreCase("_")) {
                    if (i % 2 != 0) {
                        gameboard.position[j][k] = "X";
                        return;
                    } else {
                        gameboard.position[j][k] = "O";
                        return;
                    }
                }
            }
